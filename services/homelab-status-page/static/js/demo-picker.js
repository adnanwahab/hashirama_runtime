...
//alan kay kerpow blue plane - pink plane
 function Example() {
  return (
    <fieldset>
      <legend className="text-sm font-semibold leading-6 text-gray-100">Select a Demo</legend>
      <div className="mt-4 divide-y divide-gray-200 border-b border-t border-gray-200">
        {sides.map((side, sideIdx) => (
          <div key={sideIdx} className="relative flex items-start py-4">
            <div className="min-w-0 flex-1 text-sm leading-6">
              <label htmlFor={side.id} className="select-none font-medium text-gray-100">
                {side.name}
              </label>
            </div>
