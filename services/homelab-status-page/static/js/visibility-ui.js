...
function interpretSvg(ctx, path) {
    for (let i = 0; i < path.length; i++) {
        if (path[i] === 'M') { ctx.moveTo(path[i+1], path[i+2]); i += 2; }
        if (path[i] === 'L') { ctx.lineTo(path[i+1], path[i+2]); i += 2; }
    }
}
...
class BlockLayer {
    constructor(diagram) {
        ...
        diagram.blocks.forEach(function(block, i) {
            block.bounds = [block.r, diagram.maxStorageColumn - block.r,
                            block.r, diagram.size - block.r];
            ...
            let gradient = ctx.createRadialGradient(block.r, block.r, 0,
    ...
    draw(ctx) {
        ...
        ctx.lineWidth = 4;
        ctx.beginPath();
        this.diagram.visibility.demo_intersectionsDetected.forEach(function (segment) {
    ...
    hitTest(location) {
        for (let block of this.diagram.blocks) {
            if (block.x - block.r <= location.x && location.x <= block.x + block.r &&
                block.y - block.r <= location.y && location.y <= block.y + block.r) {
                return block;
            }
        }
        return null;
...
class CenterLayer {
    constructor(diagram) {
        ...
        this.diagram.center.bounds =
            [diagram.gridMargin, diagram.size - diagram.gridMargin,
             diagram.gridMargin, diagram.size - diagram.gridMargin];
    }
    ...
    hitTest(location, slack=1) {
        ...
        if (center.x - size <= location.x && location.x <= center.x + size &&
            center.y - size <= location.y && location.y <= center.y + size) {
            return center;
        }
        return null;
    }

    hitTestTight(location) { return this.hitTest(location, 1.5); }
    hitTestLoose(location) { return this.hitTest(location, 5); }
}
...
class SliderWidget {
    constructor(diagram) {
        ...
        if (diagram.options.slider) {
            ...
            input.setAttribute('value', diagram.options.slider === 'right'? this.paddedMax : this.min);
            ...
            input.addEventListener('touchmove', event => {
                ...
                value = clamp(this.min + value * (this.paddedMax - this.min), this.min, this.paddedMax);
                input.value = value;
    ...
    get() {
        return this.slider? clamp(this.slider.valueAsNumber, this.min, this.max) : this.max;
    }
    ...
    fade() {
        ...
        return clamp((value - (this.max - this.padding)) / (2 * this.padding), 0, 1);
    }
    ...
    return class BackgroundLayer {
        ...
        draw() {
            ...
            if (this.diagram.drawLights) {
                ...
                v.loadMap(
                    size, this.diagram.gridMargin, this.diagram.blocks,
                    this.diagram.walls.map(wall => ({p1: {x: wall[0], y: wall[1]},
                ...
                lights.forEach(function (p) {
                    ...
                    let gradient = ctx.createRadialGradient(p[0], p[1], 0, p[0], p[1], options.lightRadius || 100);
...
function makeDiagram(rootId, options, backgroundClass, center, blocks, walls) {
    ...
    // Canvas layers
    let state = {root, options, size, maxStorageColumn, gridMargin, center, blocks, walls, visibility, drawLights: false, update: updateAll, highlight: null};
    ...
    root.addEventListener('touchmove', event => { event.preventDefault(); });
    ...
    visible.addEventListener('mousemove', event => {
        ...
        setHighlight(centerLayer.hitTestTight({x, y}) || blockLayer.hitTest({x, y}));
    });
    ...
    makeDraggable(visible, visible, (begin, current, state) => {
        ...
        if (!state.dummy) {
            state.x = clamp(state.offsetX + x, state.bounds[0]+epsilon, state.bounds[1]-epsilon);
            state.y = clamp(state.offsetY + y, state.bounds[2]+epsilon, state.bounds[3]-epsilon);
            updateAll();
        }
        return state;
    });
    ...
    function redrawAll() {
        visibility.loadMap(size, gridMargin,
                           ...
                           walls.map(wall => ({p1: {x: wall[0], y: wall[1]},
                                               p2: {x: wall[2], y: wall[3]}}))
                          );
    ...
    function drawFloor(ctx, path, solidStyle) {
        let gradient = ctx.createRadialGradient(center.x, center.y, 0,
        ...
        if (state.drawLights) {
            ...
            ctx.moveTo(0, 0); ctx.lineTo(0, size); ctx.lineTo(size, size); ctx.lineTo(size, 0);
        }
        ctx.fill();
        ctx.restore();
    }
    ...
    function drawTriangleLabels(ctx, path) {
        ...
        for (let i = 0; i < path.length; i += 3) {
            let x1 = path[i+1], y1 = path[i+2],
                x2 = path[i+4], y2 = path[i+5];
            let xc = (center.x + x1 + x2) / 3,
                yc = (center.y + y1 + y2) / 3;
            let area = Math.abs((x1 * (y2-yc) + x2 * (yc-y1) + xc * (y1-y2)) / 2);
    ...
    function drawEndpoints(ctx) {
        ...
        while (i.hasNext()) {
            let p = i.next();
            if (p.visualize) {
                ctx.fillStyle = (p.angle < maxAngle + 1e-2)? 'hsl(60, 80%, 50%)' : 'hsl(60, 20%, 40%)';
                ctx.beginPath();
                ctx.arc(p.x, p.y, (Math.abs(p.angle - maxAngle) < 1e-2)? 5 : 3, 0, 2*Math.PI, true);
                ctx.fill();
                ctx.fillStyle = "white";
    ...
    function drawSweepLine(ctx) {
        ...
        ctx.beginPath();
        ctx.moveTo(center.x, center.y);
        ctx.lineTo(center.x + size * Math.cos(maxAngle),
...
function makeOtherDiagrams() {
    ...
    makeDiagram(
        ...
        {slider: 'left', drawEndpoints: true, labelEndpoints: true, drawSegments: false},
        makeBackgroundLayer(),
        {x: 100, y: 300, r: 15},
        [
            {x: 120, y: 100, r: 40},
    ...
    let grenade = makeDiagram(
        'grenade',
        {},
        makeBackgroundLayer([[130, 80], [150, 305], [280, 130]],
